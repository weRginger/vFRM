...
class g:
   ...
   STR_FILTER = -1   #suspend I/O statistic from the #th epoch
   END_FILTER = -1   #suspend I/O statistic to the #th epoch
...
def main():
   ...
   if (len(args) == 4):
      g.STR_FILTER = int(args[2])
      g.END_FILTER = int(args[3])
      assert(g.STR_FILTER > 0 and g.STR_FILTER < g.END_FILTER)
   ...
   while True:
      ...
      if ioTime < curEpoch * g.REPLACE_EPOCH:
         if (g.STR_FILTER <= curEpoch <= g.END_FILTER):    #Bypass spike I/Os
            continue
         ...
         if endEpoch > g.TRAININGTIME:
            if strEpoch <= g.TRAININGTIME:
               ...
               if g.STR_FILTER <= curEpoch <= g.END_FILTER:    #Bypass spike I/Os statistic
                  gap = numGap
         ...
         if not (g.STR_FILTER <= curEpoch <= g.END_FILTER):    #statistic only for non-spike epoch
            PopRecordByEpoch()
            PopPredNextEpoch()
