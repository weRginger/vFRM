...
def main():
   # Check for arguments
   try:
      opts, args = getopt.getopt(sys.argv[1:], "he:g:w:", ["help", "epoch=", "global=", "write="])
...
def CheckCacheHit(startBinID, binNum, n):
   ...
   for i in xrange(binNum):
      ...
      CacheUpdateIOCost(cacheHit, binID, g.wl[n].ioRW)
      if not cacheHit:
         flagHit = False
   if flagHit:
      g.cache.numHit += 1
...
def CacheUpdateIOCost(cacheHit, binID, ioRW):
   if ioRW == 'R':
      #no matter read hit or miss, no flash update, thus no IO cost
      pass
   elif ioRW == 'W' and g.writePolicy == 0:
      if cacheHit:
         g.cache.SetDirty(binID)
      else:
         #In Write Back, if write miss, write I/O goes to HDD w/o flash update
         pass
   elif ioRW == 'W' and g.writePolicy == 1:
      if cacheHit:
         g.numEvict += 1
      else:
         #In Write Through, if write miss, write I/O goes to HDD w/o flash update
         pass
   else:
...
def StatByEpoch(epoch, gap):
   ...
   if g.numWL == 1:
      ...
      with open(os.path.join(g.dirPath, '%s-StatByEpoch-%dmin-%dMB-%s' % (g.dirName, g.REPLACE_EPOCH/60, g.FLASH_SIZE, g.wrAbbr)), 'a') as source:
   ...
   #for the cases with multiple workloads
      ...
      with open(os.path.join(g.dirPath, 'StatByEpoch-%dfile-%dmin-%dMB-%s-glb%d' % (g.numWL, g.REPLACE_EPOCH/60, g.FLASH_SIZE, g.wrAbbr, g.glbPolicy)), 'a') as source:
         keyInCache = g.cache.binInCache.keys()
         ...
         for i in xrange(g.numWL):
            ...
            #---------------------------------------
            with open(os.path.join(g.dirPath, '%s-StatByEpoch-%dfile-%dmin-%dMB-%s-glb%d' % (g.wl[i].fname, g.numWL, g.REPLACE_EPOCH/60, g.FLASH_SIZE, g.wrAbbr, g.glbPolicy)), 'a') as subSource:
               if g.glbPolicy == 0:
...
def GetFlashShare(epoch, gap):
   ...
   for i in xrange(g.numWL):
      ...
      sum += sharePct[i]
      shareSum[i] = sum
   with open(os.path.join(g.dirPath, 'Flashshare-%dfile-%dmin-%dMB-%s-glb%d' % (g.numWL, g.REPLACE_EPOCH/60, g.FLASH_SIZE, g.wrAbbr, g.glbPolicy)), 'a') as source:
      for i in xrange(gap):
...
def PrintSummary():
   ...
   print '|    IO admin num: %d' % (g.numAdmin)
   print '|    IO evict num: %d' % (g.numEvict)
   print '|    Total IO cost: %d' % (g.numAdmin + g.numEvict)
   print '|    Cache hit ratio: %.4f%%' % (float(g.cache.numHit) / g.cache.numIO * 100)
   print '|--------------------------------------------|'

   if g.numWL == 1:
      outFile = open(os.path.join(g.dirPath, '%s-Summary-%dmin-%dMB-%s' % (g.dirName, g.REPLACE_EPOCH/60, g.FLASH_SIZE, g.wrAbbr)), 'a')
      ...
      outFile.write('IO admin num: %d\n' % (g.numAdmin))
      outFile.write('IO evict num: %d\n' % (g.numEvict))
      outFile.write('Total IO cost: %d\n' % (g.numAdmin + g.numEvict))
      outFile.write('Cache hit ratio: %.4f%%\n' % (float(g.cache.numHit) / g.cache.numIO * 100))
      outFile.write('\n')
      outFile.close()
   else:
      outFile = open(os.path.join(g.dirPath, 'Summary-%dfile-%dmin-%dMB-%s-glb%d' % (g.numWL, g.REPLACE_EPOCH/60, g.FLASH_SIZE, g.wrAbbr, g.glbPolicy)), 'a')
      outFile.write('Flash size: %d(MB)\n' % g.FLASH_SIZE)
      outFile.write('Write policy: %s\n' % (writing))
      outFile.write('Number of I/Os: %d\n' % g.cache.numIO)
      outFile.write('Number of Read: %d\n' % g.cache.numRead)
