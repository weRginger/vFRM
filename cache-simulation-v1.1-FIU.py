...
class g:
   ...
   CACHE_LINE = 8 #cache line size, default value is 8 Blocks = 4KB
   SLOT_NUM = 0   #number of slot, (SLOT_NUM = CACHE_SIZE / CACHE_LINE)
...
def main():
   ...
   if len(sys.argv) != 6:
      print 'Usage: %s [cache size] [mapping strategy] [replacement strategy] [write strategy] [trace file]' % os.path.basename(sys.argv[0])
      sys.exit(1)

   #Read arguments from command line
   g.CACHE_SIZE = long(sys.argv[1])
   g.SLOT_NUM = g.CACHE_SIZE / g.CACHE_LINE
