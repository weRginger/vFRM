...
def GetTraceReference(inFile, lineNum):
   ...
   if line != '':
      ...
      ioLBN = int(line[5], 16) / 512
      ioSize = int(line[6], 16) / 512
      ioRW = line[0]
      if ioRW == 'DiskWrite':
         ioRW = 'W'
      elif ioRW == 'DiskRead':
...
def PopStat(ioLBN, ioSize, ioRW):
   ...
   if ioRW == 'W':
      for i in xrange(binNum):
         binID = startBinID + i
         if binID in g.popWrite:
            g.popWrite[binID] += 1 / float(binNum)
         else:
            g.popWrite[binID] = 1 / float(binNum)
   else:
      for i in xrange(binNum):
         binID = startBinID + i
         if binID in g.popRead:
            g.popRead[binID] += 1 / float(binNum)
         else:
            g.popRead[binID] = 1 / float(binNum)
...
def ReaccStat(ioLBN, ioSize, ioTime):
   ...
   for i in xrange(binNum):
      ...
      if g.reaccIO[binID][2] is None:
         ...
         timeDiff = (ioTime - g.reaccIO[binID][2]) / 60 + 1    #min as time granularity
   ...
   for binID in g.reaccIO:
      if g.reaccIO[binID][1] != 0:
         ...
         for timeDiff in g.reaccIO[binID][0]:
            ...
            elif timeDiff <= TG_5MIN:
               tmpDict[binID][2] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_10MIN:
               tmpDict[binID][3] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_15MIN:
               tmpDict[binID][4] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_20MIN:
               tmpDict[binID][5] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_25MIN:
               tmpDict[binID][6] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_30MIN:
               tmpDict[binID][7] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_40MIN:
               tmpDict[binID][8] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_50MIN:
               tmpDict[binID][9] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_60MIN:
               tmpDict[binID][10] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_70MIN:
               tmpDict[binID][11] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_23HR:
               tmpDict[binID][12] += g.reaccIO[binID][0][timeDiff]
            elif timeDiff <= TG_25HR:
               tmpDict[binID][13] += g.reaccIO[binID][0][timeDiff]
            else:
               tmpDict[binID][14] += g.reaccIO[binID][0][timeDiff]
...
def StatByMinute(ioLBN, ioSize, ioTime, ioRW):
   ...
   if minuteGap >= 1:
      ...
      currTimeDict[currLastMinute][3] = currTimeDict[currLastMinute][3] / 2   # KB #
      currTimeDict[currLastMinute][4] = currTimeDict[currLastMinute][4] * 4   # KB #
   ...
   for i in xrange(binNum):
      ...
      if binID not in currBinDict:
         ...
         currTimeDict[currLastMinute][5] += 1   #total accessed size in granularity of MB
   for i in xrange(slotNum):
      slotID = startSlotID + i
      if slotID not in currSlotDict:
         currSlotDict[slotID] = True
         currTimeDict[currLastMinute][4] += 1   #total accessed size in granularity of 4KB
