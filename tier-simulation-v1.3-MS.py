...
def main():
   ...
   trainingTime = 3600    #second, the traces in the 1st day is used for flash warm-up
   ...
   while True:
      ...
      if ioTime < numEpoch * g.REPLACE_EPOCH:
         ...
         if numEpoch * g.REPLACE_EPOCH > trainingTime:   #second, the traces in the 1st day is used for flash warm-up
         ...
         if numEpoch * g.REPLACE_EPOCH <= trainingTime and (numEpoch + numGap) * g.REPLACE_EPOCH > trainingTime:
            gap = (numEpoch + numGap) - trainingTime/g.REPLACE_EPOCH - 1;
            with open(os.path.join(g.dirPath, '%s-StatByEpoch-%dmin-%dMB' % (g.dirName, g.REPLACE_EPOCH/60, g.FLASH_SIZE)), 'a') as source:
               for i in xrange(gap):
                  source.write('%d\t0\t0\t0\t0\t0.0\t%d\t0\t0\t0.0\t0\t0\t0.0\n' % ((trainingTime+(i+1)*g.REPLACE_EPOCH)/60, len(g.cache.binInCache)))
         elif numEpoch * g.REPLACE_EPOCH > trainingTime:
